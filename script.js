...
const PRESETS = {
    'south': ['idli', 'dosa', 'rice', 'dal', 'coffee', 'f1 shake'],
    'north': ['roti', 'dal', 'paneer', 'tea', 'rice', 'f1 shake'],
    'modern': ['sandwich', 'egg', 'chicken', 'coffee', 'f1 shake'],
...
let currentSlot = '';
const COACH_PLAN = {
    "Monday": { sn1: "Almonds", lun: "Brown Rice + Dal", sn2: "Green Tea", vCal: 340, t: "Body Basics", vid: "b9ztxh-cTHI" },
    "Tuesday": { sn1: "Apple", lun: "Ragi Mudde", sn2: "Buttermilk", vCal: 320, t: "Core Strength", vid: "ELpVLwrDR_g" },
    "Wednesday": { sn1: "Egg", lun: "Millet Rice", sn2: "Papaya", vCal: 310, t: "Metabolism", vid: "VsAXZ34AJ-k" },
    "Thursday": { sn1: "Cucumber", lun: "Jowar Roti", sn2: "Black Coffee", vCal: 330, t: "Lower Body", vid: "rmmWuI5Jinc" },
    "Friday": { sn1: "Guava", lun: "Curd Rice", sn2: "Protein Bar", vCal: 300, t: "Cardio Kick", vid: "8r4Z-ghDI4k" },
    "Saturday": { sn1: "Coconut Water", lun: "Veg Biryani", sn2: "Makhana", vCal: 400, t: "Stability", vid: "uKXcLBjWqas" },
...
function goToLifestyle() {
    ...
    if(!n || !w) return alert("Please fill Name & Weight");
...
function finishSetup(style) {
    ...
    state = { 
        name: n, weight: w, height: h, startW: w, goal: idealW.toFixed(1), 
...
function init() { 
    ...
    checkNewDay(); updateUI(); renderMeals(); renderTasks();
}
...
function updateUI() {
    ...
    let total = state.startW - state.goal;
    ...
    if(diff <= 0) document.getElementById('daysToGoal').innerText = "Done!";
         ...
         let rate = lost > 0 ? (lost / Math.max(1, (Date.now()-state.startDate)/(86400000))) : 0.05;
         let d = Math.ceil(diff/rate);
...
function checkNewDay() { const t = new Date().toISOString().split('T')[0]; if(state.lastLogin!==t) { state.lastLogin=t; state.waterC=0; state.consumed=[]; state.todayTasks={}; save(); } }
...
function shareWhatsApp() {
    ...
    const text = `*ZestUp Pro Update* 🚀%0a🔥 Calories: ${total}/${state.calGoal}%0a⚖️ Weight: ${state.weight}kg%0a💧 Water: ${state.waterC}ml`;
...
function quickLog(name, cal) { state.consumed.unshift({ name: name, cal: cal, qty: 1, time: Date.now() }); save(); updateUI(); alert(`Added ${name}`); }
