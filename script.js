...
window.onload = async () => {
    ...
    if (!state) { 
        ...
        // If new user, hide loader so they can see setup
        document.getElementById('loading-screen').classList.add('hide');
    } else { 
        if(state.customFoods) foodDB = { ...foodDB, ...state.customFoods }; 
        init(); 
        
        // 3. DATABASE READY: Remove Loading Screen with a slight delay for smoothness
        setTimeout(() => {
            document.getElementById('loading-screen').classList.add('hide');
        }, 500);
    }
...
async function loadFoodData() {
    try {
        // Cache Buster V13.4
        const response = await fetch('food.json?v=13.4');
        ...
        // Hide loader even on error so they aren't stuck forever
        document.getElementById('loading-screen').classList.add('hide');
    }
}
...
function browseCategory(cat) {
    // Logic is now safe because loader prevents early clicking
    ...
    items.forEach(keyword => {
        ...
        if(matchKey) {
            ...
            html += `<div onclick="selectFood('${matchKey}')" class="bg-white p-4 rounded-2xl border border-slate-50 shadow-sm flex justify-between items-center cursor-pointer mb-2 btn-click">
                        <div><p class="font-bold text-marine capitalize text-sm">${matchKey}</p><p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Tap to add</p></div>
                        <div class="w-8 h-8 rounded-full ${badgeColor} flex items-center justify-center"><i class="fas fa-plus"></i></div>
                     </div>`;
        }
    });

    if (foundCount === 0) html = `<div class="text-center py-10 text-slate-400 font-bold text-xs">Loading items...</div>`;
