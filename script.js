...
window.onload = async () => {
    ...
    if (!state) { 
        ...
        init(); 
        setTimeout(() => { document.getElementById('loading-screen').classList.add('hide'); }, 500); 
    }
};
...
async function loadFoodData() {
    try {
        ...
        console.error(error);
        alert("Network Error: Could not load food database."); 
    }
}
...
function togglePref(pref) {
    const btn = document.getElementById('pref-' + pref);
    if (tempPrefs.includes(pref)) {
        tempPrefs = tempPrefs.filter(p => p !== pref);
        btn.classList.remove('border-punch', 'bg-orange-50');
        btn.classList.add('border-slate-100', 'bg-mist');
        btn.querySelector('i').className = btn.querySelector('i').className.replace('text-punch', 'text-slate-300');
        btn.querySelector('span').className = "text-xs font-bold text-slate-400";
    } else {
        tempPrefs.push(pref);
        btn.classList.remove('border-slate-100', 'bg-mist');
        btn.classList.add('border-punch', 'bg-orange-50');
        btn.querySelector('i').className = btn.querySelector('i').className.replace('text-slate-300', 'text-punch');
        btn.querySelector('span').className = "text-xs font-bold text-punch";
    }
}
...
function finishSetup() {
    ...
    if(tempPrefs.includes('south')) myPicks.Breakfast.unshift('idli', 'dosa', 'upma');
    if(tempPrefs.includes('north')) { myPicks.Breakfast.unshift('paratha'); myPicks.Lunch.unshift('roti', 'curry'); }
    if(tempPrefs.includes('rice')) myPicks.Lunch.unshift('rice', 'sambar');
    if(tempPrefs.includes('nonveg')) { myPicks.Lunch.unshift('chicken', 'fish'); myPicks.Dinner.unshift('chicken'); }
...
function openSmartSelector(mealType) {
    ...
    const gridHTML = quickItems.map(itemKey => {
        ...
        return `<div onclick="selectFood('${itemKey}'); closeSmartSelector()" class="flex flex-col items-center gap-2 p-3 rounded-xl border border-slate-50 hover:border-punch transition-all cursor-pointer bg-white"><div class="w-8 h-8 rounded-full ${colorClass} flex items-center justify-center text-xs"><i class="fas fa-utensils"></i></div><span class="text-[10px] font-bold text-marine capitalize">${itemKey}</span></div>`;
    }).join('');

    const modalHTML = `
        <div id="smartSelectorModal" class="fixed inset-0 z-[600] bg-marine/90 backdrop-blur-md flex items-end sm:items-center justify-center p-4 animate-fade-in">
            <div class="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl space-y-4 animate-slide-up relative h-[80vh] flex flex-col">
                <div class="flex justify-between items-center"><h3 class="text-xl font-black uppercase tracking-tight text-marine">Select ${mealType}</h3><button onclick="closeSmartSelector()" class="w-8 h-8 bg-mist rounded-full flex items-center justify-center text-slate-400"><i class="fas fa-times"></i></button></div>
                <div class="relative"><input type="text" id="smartSearch" onkeyup="showSmartSuggestions(this.value)" placeholder="Type to search..." class="w-full bg-mist p-4 pl-12 rounded-2xl outline-none font-bold text-marine border border-slate-100"><i class="fas fa-search absolute left-5 top-5 text-slate-300"></i></div>
                <div id="smartSearchResults" class="hidden space-y-2 overflow-y-auto max-h-40"></div>
                <div id="smartContextGrid"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Your Favorites</p><div class="grid grid-cols-3 gap-2 overflow-y-auto pb-4">${gridHTML}</div></div>
            </div>
        </div>`;
    document.body.insertAdjacentHTML('beforeend', modalHTML);
    setTimeout(() => document.getElementById('smartSearch').focus(), 100);
}

function closeSmartSelector() { const m = document.getElementById('smartSelectorModal'); if(m) m.remove(); }
function showSmartSuggestions(val) {
    const list = document.getElementById('smartSearchResults');
    const grid = document.getElementById('smartContextGrid');
    if (val.length < 1) { list.classList.add('hidden'); grid.classList.remove('hidden'); return; }
    ...
    if (matches.length === 0) { list.innerHTML = `<div onclick="saveCustomFromSmart('${val}')" class="p-3 border-b border-slate-50 text-center text-xs font-bold text-punch cursor-pointer">+ Add "${val}" to DB</div>`; } 
    else { list.innerHTML = matches.map(f => `<div onclick="selectFood('${f}'); closeSmartSelector()" class="p-3 border-b border-slate-50 hover:bg-mist cursor-pointer flex justify-between items-center"><span class="font-bold text-marine capitalize text-sm">${f}</span></div>`).join(''); }
}
...
function renderMeals() {
    ...
    if (currentMenuMode === 'user') {
        document.getElementById('mealContainer').innerHTML = `${renderSearchSlot('Breakfast', 'Tap to select...')}${renderSearchSlot('Mid-Snack', 'Tap to select...')}${renderSearchSlot('Lunch', 'Tap to select...')}${renderSearchSlot('Eve-Snack', 'Tap to select...')}${renderSearchSlot('Dinner', 'Tap to select...')}`;
    } else {
...
function renderSearchSlot(title, ph) {
    ...
    return `<div class="bg-mist p-4 rounded-[2rem] flex justify-between items-center border border-slate-50 shadow-sm relative overflow-hidden group"><div class="flex items-center gap-4 cursor-pointer" onclick="openSmartSelector('${title}')"><div class="bg-white w-10 h-10 rounded-full flex items-center justify-center text-slate-400 shadow-sm"><i class="fas fa-plus"></i></div><div><p class="text-[9px] font-bold text-slate-400 uppercase mb-0.5">${title}</p><p class="font-extrabold text-sm text-marine">${ph}</p></div></div>${swapBtn}</div>`;
}
...
function showAddCustom(val) { document.getElementById('custName').value = val; document.getElementById('addCustom').classList.remove('hide'); }
